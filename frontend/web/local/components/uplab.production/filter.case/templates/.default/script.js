...
var jsFilterCase = {
    ...
    */
    'removeTag' : function (elm) {
        ...
        //Применяем фильтр со снятым свойством
    ...
    */
    'filterApply': function (clearAll = false) {
        ...
        //Данные select с формы
        var formData = JSON.stringify($('#form-case-filter').serializeArray());



        //Выбранный таб
        var tabSelect = $(".tabs-list__wrap .tabs-list__el.active").data("href");


        if(tabSelect === undefined){

            var mobileSelect = $(".tabs-list .js-select2").val();
            if(mobileSelect !== undefined){
                tabSelect = mobileSelect;
            }
        ...
        BX.ajax.runComponentAction('uplab.production:filter.case',
            'sendForm', {
                mode: 'class',
                data: {
                    post: {
    ...
    * Список тегов формируется в комоненте
    * Теги состоят из кода свойства (data-prop) и id элмента списка свойства (data-id)
    * По ним происходит удаление тега
    * */
